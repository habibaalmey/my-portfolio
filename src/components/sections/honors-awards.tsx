...
export function HonorsAwardsSection() {
  return (
    <SectionContainer id="honors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16 pt-8"> {/* Increased gap-y and pt-8 for bulb space */}
        {honorsAndAwards.map((item, index) => (
          ...
          >
            <Card className={cn(
              "flex flex-col text-center h-full card-lightbulb relative pt-8", // Added pt-8 for bulb container
              "transform transition-all duration-300 ease-in-out hover:scale-[1.03]" // General hover effect
            )}>
              <div className="lightbulb-icon-container">
                <item.icon />
              </div>
              <div className="lightbulb-glow absolute inset-0 rounded-lg pointer-events-none -z-10"></div> {/* Glow element */}
